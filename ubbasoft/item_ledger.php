...
include('config.php');    
$query="SELECT *  from  newitem_reg where product_type='2'";
    ...
    <div class="qq">
      <div class="panel panel-default">
        <div class="panel-heading">
          <strong>
            <h1 style="text-align: center; color: #c4cfe4; text-shadow: 2px 2px 5px green;" ><span class="glyphicon glyphicon-th"></span>Item Ledger</h1>
         </strong>
        </div>
    <div class="panel-body">
          <form action="item_ledger.php" method="post" >
...
?>
<div class="qq">
   <a href="print_item_ledger.php?fd=<?php echo $fd?>&ed=<?php echo $ed?>&spid=<?php echo $spid?>" class="btn btn-success" style="float: right;"><span class="glyphicon glyphicon-open"></span>&nbspPrint</a>
<div class="panel panel-default">
...
$result22 = $conn->query("SELECT * FROM production_file where product_id= '$sale_person_id' and prod_date between '$from' and '$to'");


// $result221 = $q->query("SELECT * FROM invoice_detail d, invoice i where i.dc_date between '$from' and '$to' AND d.prod_id = '$sale_person_id'");

$result221 = $conn->query("SELECT * FROM invoice_detail d, invoice i where d.invoice_id=i.invoice_no and i.dc_date between '$from' and '$to' AND d.prod_id = '$sale_person_id'");
...
$result221x = $conn->query("SELECT product_title FROM newitem_reg where id = '$sale_person_id'");
$row3=$result221x->fetch_array(MYSQLI_NUM);


$result2214x = $conn->query("SELECT uom_name FROM uom u, newitem_reg n where n.uom=u.id and n.id = '$sale_person_id'");
$row4=$result2214x->fetch_array(MYSQLI_NUM);
...
if($row=$result22->fetch_array(MYSQLI_NUM))
{
  ?>
                ...
                <td class="text-center"> <?php echo remove_junk($row4[0]); ?></td>
