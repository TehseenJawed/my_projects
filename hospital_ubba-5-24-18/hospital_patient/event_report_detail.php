...
{
  $events=$_GET['events'];
  $sql="SELECT * FROM p_events WHERE events='$events'";
...
{
  $name=$_GET['name'];
  $sql="SELECT * FROM p_events WHERE name='$name'";
  $result1 = mysqli_query($con, $sql);
...
{
  $c_type=$_GET['c_type'];
  $sql="SELECT * FROM p_events where c_type ='$c_type'";
  $result1 = mysqli_query($con, $sql);
}
...
if(isset($_GET['dis']))
{
  ...
  $dis=$_GET['dis'];
  $sql="SELECT * FROM p_events where disease ='$dis'";
  $result1 = mysqli_query($con, $sql);
}
...
{
  $p_num=$_GET['p_num'];
  $sql="SELECT * FROM p_events where p_num ='$p_num'";
  $result1 = mysqli_query($con, $sql);
}
...
if(isset($_GET['f_date'])&& isset($_GET['to_date']))
{
  $f=$_GET['f_date'];
  $to=$_GET['to_date'];
  $sql="SELECT * FROM p_events where datee BETWEEN '$f' ANd '$to'";
  $result1 = mysqli_query($con, $sql);
}
